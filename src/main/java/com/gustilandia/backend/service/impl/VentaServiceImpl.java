...
public class VentaServiceImpl implements VentaService {
	...
	public Response registrar(DTOVentas dtoVenta) {
		...
		try {
			...
			for (VentaDetalle det : venta.getVentaDetalle()) {
				...
				if (ventarepo.validarStock(det.getCantidad(), producto.getIdProducto()) == 0) {
					stock = false;
					errorStock.add("No hay suficiente Stock de " + producto.getProducto());
				}
			}
	...
	public Response listarVentasPorRol(Long idRol, Long idUsuario) {
		...
		List<DTOListadoVenta> listadoVentas = ventarepo.listadoVentas();

		if (idRol == 2) {
			listadoVentas = listadoVentas.stream().filter(venta -> venta.getIdEstado() == 6)
					.collect(Collectors.toList());
		}

		if (idRol == 5) {
			listadoVentas = listadoVentas.stream().filter(
					venta -> venta.getIdEstado() == 7 && venta.getIdEmpleado() == idUsuario)
					.collect(Collectors.toList());
		}
	...
	@Override
	public Response listarVentaPorCliente() {
		...
		List<DTOVentaPorCliente> listado = ventarepo.findByCliente(cliente.getIdCliente());

		return new Response(true, listado, "");
	}

	@Override
	public Response VentaPorCliente() {
		String token = TokenClientInterceptor.token;
		token = token.replace("Bearer ", "");
		String correo = jwtProvider.getUsuarioToken(token);
		Cliente cliente = clienterepo.findByCorreo(correo).get();
		
		List<DTOVentaCliente> listado = ventarepo.ventaCliente(cliente.getIdCliente());
		return new Response(true, listado, "");
	}

	@Override
	public Response DetalleVentaCliente(Long idVenta) {
		
		List<DTODetallVentaCliente> listado = ventarepo.detalleVentaCliente(idVenta);
		return new Response(true, listado, "");
	}
